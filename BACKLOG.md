...
| BTL-B62 | Help button in the header — `HelpSheet` with *How BetaLog works* and *Send feedback*, both header variants. Release B of `betalog_help_and_feedback_spec.md`; mock-up approved 2026-09-24 | Feature | Session | Blocked | BTL-B15, and whether `BenjuiceyFeedback.open()` takes a category (spec §5) |
...
| ID | Item | Closed |
|---|---|---|
| BTL-B61 | The guide — `/help.html`, fourteen chapters explaining every screen in plain words with the grade-and-goals material in chapter 5, cross-linked with the explainer, a feedback button on the page, and a pre-merge checklist line so a visible UI change updates it in the same commit. Written from three code sweeps, not the specs; verified in Chromium at 390 and 1280 px, every anchor resolving, the feedback button opening the widget. Built and verified **on the branch `claude/feedback-help-prominence-50mjd8`**, released when Ben says | 2026-09-24 |
