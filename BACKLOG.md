...
| BTL-B55 | Complete! card on a phone — *Rope Grade · Send 6c* in its slot on Plan › Goals with no Achieved list under it; *Set your next goal* opens the sheet on Rope at 6c+; setting it removes the card; the green row in History still deletes; **merge first** — on the branch `claude/sync-not-automatic-wycyw1`, not released | Check | **Ben** | Ready | — |
