...
- **Declutter / information architecture rework** — planned in `docs/specs/betalog_ia_declutter_spec.md`. Four phases, each independently shippable: 0) ✅ **done 2026-08-20** — deleted dead `GoalsWidget`, extracted BMI logic to `stats.js`; 1) ✅ **done 2026-08-20** — Schedule is its own Plan tab, calendar setup moved there from Settings; was: Schedule becomes its own Plan tab, calendar setup moves out of Settings; 2) shared collapsible widget shell with state persisted in `profile.widgetCollapsed`; 3) dedup — grade charts fold into `LevelCard`, `ClimbingStats` leaves Profile, weight/BMI readout stops being in two places — **and Profile becomes Goals** (decision 4, 2026-08-20: the dashboard widget picker joins `widgetOrder` in the Dashboard's Edit layout mode, name/height go to Settings, leaving Plan as `Schedule | Routines | Exercises | Goals`). All three open decisions are now settled (collapse state syncs via `profile`; charts default collapsed and numbers expanded; calendar setup moves to the Schedule tab) — nothing blocks the build.
